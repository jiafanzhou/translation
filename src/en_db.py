...
class MyDictionary(object):
    ...
    def print_network_translate(self):
        if self.networkTranslation != None:
...
def persist_word():
    try:
        date_id = time.strftime("%m/%d/%Y")
        timestamp = time.strftime("%c")
        
        sql = ("insert into words (date_id, time_id, word,"
            + "category) values (\'" + date_id + "\'"  
            + ",\'" + timestamp + "\'"
            + ",\'" + re.escape(mydict.word) + "\'"
            + ", \'default\'" + ")")
        
        
        con = psycopg2.connect(database='en_words', user='ejiafzh')
        con.autocommit = True 
        cur = con.cursor()
        cur.execute(sql)
        
        print "word: " + mydict.word + " saved into the book"
